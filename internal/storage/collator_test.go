...
func TestCollator_Collate(t *testing.T) {
	...
	c.Assert(specs[v20220401_ga].Paths.Value("/example").Post.Extensions["x-other-internal"], qt.Not(qt.IsNil))
	c.Assert(specs[v20220401_ga].Paths.Value("/example").Post.
		Responses.Status(204).Value.Extensions["x-internal"], qt.Not(qt.IsNil))
}
...
func TestCollator_Collate_MigratingEndpoints(t *testing.T) {
	...
	c.Assert(specs[v20220201_exp].Paths.Value("/test").Get.Responses.Status(204), qt.IsNotNil)
	c.Assert(specs[v20220201_exp].Paths.Value("/test").Get.Responses.Status(200), qt.IsNil)

	c.Assert(specs[v20230314_exp].Paths.Value("/test").Get.Responses.Status(200), qt.IsNotNil)
	c.Assert(specs[v20230314_exp].Paths.Value("/test").Get.Responses.Status(204), qt.IsNil)
}
...
func TestCollator_Collate_ExcludePatterns(t *testing.T) {
	...
	c.Assert(specs[v20220401_ga].Paths.Value("/example").Post.Extensions["x-other-internal"], qt.IsNil)
	c.Assert(specs[v20220401_ga].Paths.Value("/example").Post.
		Responses.Status(204).Value.Extensions["x-internal"], qt.IsNil)
}
...
func TestCollator_Collate_Conflict(t *testing.T) {
	...
	c.Assert(err, qt.IsNil)
	// First path wins
	c.Assert(specs[vervet.MustParseVersion("2021-06-15")].Paths.Value("/examples/hello-world").Post.Description,
