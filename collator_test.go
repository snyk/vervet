...
func TestCollator(t *testing.T) {
	...
	c.Assert(
		result.Paths.Find("/orgs/{orgId}/projects").
	...
	schemaRef := result.
		Paths.Find("/examples/hello-world/{id}").
		Get.
	...
	projectConflictRef := result.Paths.Find("/orgs/{orgId}/projects").Get.Parameters[6]
	exampleConflictRef := result.Paths.Find("/examples/hello-world/{id}").Get.Parameters[3]
	c.Assert(projectConflictRef.Ref, qt.Not(qt.Equals), exampleConflictRef.Ref)

	projectResp400Ref := result.Paths.Find("/orgs/{orgId}/projects").Get.Responses.Status(400)
	c.Assert(projectResp400Ref.Ref, qt.Equals, "#/components/responses/400")
	exampleResp400Ref := result.Paths.Find("/examples/hello-world/{id}").Get.Responses.Status(400)
...
func TestCollateMergingResources(t *testing.T) {
	...
	result := collator.Result()
	c.Assert(result.Paths.Find("/orgs/{org_id}/projects/{project_id}").Delete.Responses.Status(204), qt.IsNotNil)
}
...
func TestCollateOperationsOnSamePath(t *testing.T) {
	...
	c.Assert(result.Paths.Find("/examples/hello-world").Get, qt.Not(qt.IsNil))
	c.Assert(result.Paths.Find("/examples/hello-world").Get.Description, qt.Contains, " - from example 1")
	c.Assert(result.Paths.Find("/examples/hello-world").Post, qt.Not(qt.IsNil))
	c.Assert(result.Paths.Find("/examples/hello-world").Post.Description, qt.Contains, " - from example 1")
	c.Assert(result.Paths.Find("/examples/hello-world").Put, qt.Not(qt.IsNil))
	c.Assert(result.Paths.Find("/examples/hello-world").Put.Description, qt.Contains, " - from example 2")
}
